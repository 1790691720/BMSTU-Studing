...
class ItemTableEditTests(Table):
    idTest = Col('id')
    Name = Col('Название')
    AverageTime = Col('Среднее время')
    UpdateLink = ExternalURLCol(' ', url_attr='urlUpdate', attr='nameUpdate')
    DeleteLink = ExternalURLCol(' ', url_attr='urlDelete', attr='nameDelete')
...
def edit_user(user):
    if request.method == 'GET':
        ...
        return render_template('edit_result.html', url_back='../users', redirect="../users")
...
def edit_delete_user(user):
    ...
    return render_template('edit_delete_user.html', idLogin=int(user), redirect="../../users")
